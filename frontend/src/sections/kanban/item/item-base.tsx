...
const ItemBase = forwardRef<HTMLLIElement, ItemBaseProps>(
  ({ task, stateProps, sx, ...other }, ref) => {
    ...
    const renderPriority = (
      <Iconify
        icon={
          (task.priority.toLowerCase() === 'low' && 'solar:double-alt-arrow-down-bold-duotone') ||
          (task.priority.toLowerCase() === 'medium' && 'solar:double-alt-arrow-right-bold-duotone') ||
        ...
        sx={{
          ...
          right: 4,
          position: 'absolute',
          ...(task.priority.toLowerCase() === 'low' && { color: 'info.main' }),
          ...(task.priority.toLocaleLowerCase() === 'medium' && { color: 'warning.main' }),
          ...(task.priority.toLowerCase() === 'high' && { color: 'error.main' }),
        }}
      />
    );
